...
def map_cortex(
    ...
    for h in ["L", "R"]:
        ...
        if "_blur" in feat_lower:
            ...
            wmBoundaryDataPath = f"{input_dir}\\{bids_id}_hemi-{h}_surf-fsnative_label-white_{feat}.func.gii"
            wmBoundarySurfacePath = f"{surf_dir}\\{bids_id}_hemi-{h}_space-nativepro_surf-fsnative_label-white.surf.gii"
            midthicknessDataPath = f"{input_dir}\\{bids_id}_hemi-{h}_surf-fsnative_label-midthickness_{feat}.func.gii"
            midthicknessSurfacePath = f"{surf_dir}\\{bids_id}_hemi-{h}_space-nativepro_surf-fsnative_label-midthickness.surf.gii"

            sfwm1mmDataPath = f"{input_dir}\\{bids_id}_hemi-{h}_surf-fsnative_label-swm1.0mm_{feat}.func.gii"
            sfwm1mmSurfacePath = (
                f"{surf_dir}\\{bids_id}_hemi-{h}_surf-fsnative_label-swm1.0mm.surf.gii"
            )
            sfwm2mmDataPath = f"{input_dir}\\{bids_id}_hemi-{h}_surf-fsnative_label-swm2.0mm_{feat}.func.gii"
            sfwm2mmSurfacePath = (
                f"{surf_dir}\\{bids_id}_hemi-{h}_surf-fsnative_label-swm2.0mm.surf.gii"
            )
            sfwm3mmDataPath = f"{input_dir}\\{bids_id}_hemi-{h}_surf-fsnative_label-swm3.0mm_{feat}.func.gii"
            sfwm3mmSurfacePath = (
