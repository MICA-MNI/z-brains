...
if [ "$Nsteps" -eq 01 ]; then status="COMPLETED"; else status="ERROR asymmetry is missing a processing step"; fi
...
echo "${id}, ${SES/ses-/}, asymmetry, $status N=$(printf "%02d" "$Nsteps")/01, $(whoami), $(uname -n), $(date), $(printf "%0.3f\n" "$eri"), ${PROC}, ${Version}" >> "${out}/micapipez_processed_sub.csv"
